...
def test_policy_output():
    ...
    state = torch.zeros(SIM_CONFIG.OBSERVATION_DIM)
...
def test_policy_output_batched():
    ...
    T = 7
    states = torch.zeros(T, SIM_CONFIG.OBSERVATION_DIM)
...
def test_sample_action():
    ...
    assert torch.all(actions < len(SIM_CONFIG.JOINT_ACTION_VELOCITIES))
...
def test_compute_action_log_probs_matches_timestep_by_timestep():
    ...
    T = 5
    states = torch.randn(T, SIM_CONFIG.OBSERVATION_DIM)
    actions = torch.stack([
        torch.randint(0, len(SIM_CONFIG.JOINT_ACTION_VELOCITIES), (len(SIM_CONFIG.JOINTS),))
        for _ in range(T)
    ])
...
def test_compute_trajectory_loss_matches_manual_reconstruction():
    ...
    trajectory = {
        "states": [[float(i)] * SIM_CONFIG.OBSERVATION_DIM for i in range(4)],
        "actions": [[0] * len(SIM_CONFIG.JOINTS) for _ in range(4)],
...
def test_run_episode():
    ...
    try:
        ...
        trajectory, loss = run_episode(env, policy, REINFORCE_CONFIG.GAMMA)
...
def test_train_batch(pool):
    ...
    with torch.random.fork_rng():
        ...
        diagnostics = train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size, seed_start=1000)
    ...
    # Each trajectory still contains exactly SIM_CONFIG.MAX_EPISODE_STEPS transitions
    assert all(length == SIM_CONFIG.MAX_EPISODE_STEPS for length in diagnostics["episode_lengths"])
...
def test_train_batch_smaller_than_worker_count(pool):
    ...
    diagnostics = train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size=2, seed_start=2000)
...
def test_train_batch_final_partial_batch(pool):
    ...
    diagnostics = train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size=1, seed_start=3000)
...
def test_train_batch_mean_return_matches_episodes(pool, monkeypatch):
    ...
    reference_returns = [
        compute_returns(trajectory["rewards"], REINFORCE_CONFIG.GAMMA)[0]
        for trajectory in trajectories
    ]

    _replay_trajectories(monkeypatch, trajectories, pool, policy_state_dict, batch_size, seed_start)

    diagnostics = train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size, seed_start)
...
def test_train_batch_gradient_matches_averaged_losses(pool, monkeypatch):
    ...
    reference_losses = [
        compute_trajectory_loss(policy, trajectory, REINFORCE_CONFIG.GAMMA)
        for trajectory in trajectories
    ]
    ...
    train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size, seed_start)
...
def test_train_batch_single_optimiser_step(pool, monkeypatch):
    ...
    train_batch(pool, policy, optimiser, REINFORCE_CONFIG.GAMMA, batch_size, seed_start=6000)
